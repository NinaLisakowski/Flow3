...
function Login({ isLoggedIn, loginMsg, setLoginStatus }) {
	...
	return (
		<div>
			<h2>{loginMsg}</h2>
			<p>
				This simulates a real login page. Here you just need to press the
				buttonIn a read application you obviously will need to add your
				credentials, and login via the server
			</p>
			<Link to="/">
		...
		return (
			<div>
				...
				{book ? (
					...
					<div>Enter ID for book to see</div>
				)}
			</div>
		);
	}
